...
const getUserSubmissionSteps = async (
  ...
  const queryData = [userId, journeyId];

  if (stepId) {
    queryData.push(stepId);
  }
...
const updateSubmissionStep = async (
  ...
  return pgpool.query(
    ...
    SET status = 'completed', updated_at = CURRENT_TIMESTAMP, data = $4
    FROM submissions s
    WHERE 
      ...
      AND ss.submission_id = $1
	    ...
	    AND ss.step_id = $3
    `,
    [submissionId, userId, stepId, data],
  );
};
