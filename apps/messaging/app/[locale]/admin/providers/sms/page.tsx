  ...
  async (props: {
    ...
    async function submitAction(formData: FormData) {
      ...
      if (providerType === "aws") {
        ...
        const formErrors: Parameters<
          typeof temporaryMockUtils.createErrors
        >[0] = [];
    ...
    async function submitProviderType(formData: FormData) {
      "use server";
      const providerUser = await AuthenticationContextFactory.getUser();
      const providerType = formData.get("providerType")?.toString();
    ...
    const accessKeyError = awsErrors.find(
      (error) => error.field === "accessKey",
    );
