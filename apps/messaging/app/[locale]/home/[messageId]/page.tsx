...
export default async function inboxMessage(props: Props) {
  ...
  return (
    <>
      ...
      <Paragraph whitespace="pre-wrap" size="sm">{message.data?.excerpt}</Paragraph>
      <Paragraph whitespace="pre-wrap" size="md">{message.data?.plainText}</Paragraph>
      <a className="govie-back-link" href={props.searchParams?.tab === "unread" ? unreadUrl.href : allUrl.href}>
        {tPanel("backLink")}
      </a>
    </>
  );
