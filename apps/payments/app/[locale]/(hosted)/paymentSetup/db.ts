...
type PaymentRequestDetails = Pick<
  PaymentRequestDO,
  "title" | "description" | "amount" | "reference" | "redirect_url"
...
export async function getPaymentRequestDetails(
  ...
  const res = await pgpool.query<PaymentRequestDetails>(
    `select pr.title, pr.description, pr.amount, pp.provider_name, pp.provider_type, pr.reference, pr.redirect_url
