...
import { getUserPaymentRequestDetails, getUserTransactionDetails } from "./db";
import { formatCurrency } from "../../utils";

export default async function () {
  const [t, { userId }] = await Promise.all([
    getTranslations("payments.Payments"),
    PgSessions.get(),
  ]);

  const [paymentRequests, transactions] = await Promise.all([
    getUserPaymentRequestDetails(userId),
    getUserTransactionDetails(userId)
  ]);
