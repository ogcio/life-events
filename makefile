...
migrate:
	...
	npm run migrate --workspace=life-events-admin
...
run-services:
	...
	"npm run dev --workspace=life-events-admin"
start-services: install-concurrently run-services
	
kill-services:
	sleep 2 && lsof -ti:8000,8001,8002,8003,8004,8005,3000,3001,3002,3003,3004,3005,3006,3010 | xargs sudo kill -9
kill-logto:
